      ...
      try {
        ...
        if (chatWithMenteeId) {
          const privateChat = chatsData.find(
            (chat) => chat.type === 'private' && chat.participants.includes(chatWithMenteeId)
          );
          if (privateChat) {
      ...
      <div className="dashboard-container">
        ...
        <div className="tab-navigation">
          ...
          <button
            ...
            onClick={() => setSelectedChat(null) || setActiveTab('chats')}
          >
            Chats
          </button>
        </div>
        ...
        <div className="tab-content">
          {activeTab === 'mentees' && (
            <div className="mentees-list">
              <h2>My Mentees</h2>
              {mentees.length === 0 ? (
                <p className="no-data-message">No mentees assigned yet.</p>
              ) : (
                <div className="cards-grid">
                  {mentees.map((mentee) => (
                    <div key={mentee.id} className="mentee-card">
                      <h3>{mentee.name}</h3>
                      <p><strong>Roll Number:</strong> {mentee.rollNumber}</p>
                      <p><strong>Attendance:</strong> {mentee.attendance}%</p>
                      <div className="card-actions">
                        ...
                        <button 
                          ...
                          onClick={() => {
                            setSearchParams({ chatWith: mentee.id });
                            setActiveTab('chats');
                          }}
                        >
                          Message
              ...
              <div className="chat-container">
                ...
                {selectedChat ? (
                  ...
                  <div className="chat-placeholder">
                    ...
                    {chats.length === 0 && (
                      <p>No chats available. Start a conversation from a mentee's profile.</p>
                    )}
                  </div>
                )}
              </div>
